...
public class bstQuery {
	static invertINDEX_BST  inverted;
	
	
public bstQuery(invertINDEX_BST  inverted ) {
...
listone=ORq(listone,listtwo);
	...
	public static LinkedList<Integer>AndQ (String Q ){
		// Create two linked lists for query processing (AND,OR)
		LinkedList<Integer> ListOne=new LinkedList<Integer> ();
		LinkedList<Integer> ListTwo=new LinkedList<Integer> ();
...
//using sarch in class inverted to find word if it exixte
		...
		boolean Detected=inverted.searchINV(Item[0].trim().toLowerCase());
		if (Detected) {	
		...
		{  // Process the remaining words in the query
			Detected=inverted.searchINV(Item[i].trim().toLowerCase());
			if (Detected) 
